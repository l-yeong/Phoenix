...
public interface TicketsMapper {
    ...
    // 회원별 티켓 코드 목록
    @Select("SELECT t.tno,t.rno,t.ticket_code,DATE_FORMAT(t.issued_at, '%Y-%m-%d %H:%i:%s') AS issued_at, t.valid,t.price,r.gno FROM tickets t "+
            " JOIN reservations r ON t.rno = r.rno WHERE r.mno =#{mno} ORDER BY t.issued_at DESC; ")
    List<Map<String,Object>> findPayloads(@Param("mno") int mno);
